 ...
 * Title:        arm_convolve_HWC_INT4_fast.c
 * Description:  Fast INT4 version of convolution
 *
 * $Date:        09. July 2018
                         ...
                         int8_t * bufferB)
{
        ...
        for (i_out_x = 0; i_out_x < dim_im_out; i_out_x++)
        {
            ...
            for (i_ker_y = i_out_y * stride - padding; i_ker_y < i_out_y * stride - padding + dim_kernel; i_ker_y++)
            {
                for (i_ker_x = i_out_x * stride - padding; i_ker_x < i_out_x * stride - padding + dim_kernel; i_ker_x++)
                {
                    ...
                    {
                        arm_int4_to_int16_reordered_no_shift
                            ((int8_t *) Im_in + (((i_ker_y * dim_im_in + i_ker_x) * ch_im_in) >> 1), pBuffer, ch_im_in);	//FIXME: THE SHIFT in a better form!!!!!
                    }
                    pBuffer += ch_im_in ;
                }
            }
    ...
    #error "INT4 Convolution Layer not supported"
